...
{ // fields that remember the view and model objects:
   ...
   { char command = reader.readCommand("Komandat (D,W,Q) dhe shuma:");
      if ( command == 'Q' ) // quit?
      { } // terminate method by doing nothing more
         ...
         { int amount = reader.readAmount();
            ...
            else { writer.showTransaction("Depozitim ilegal ", amount); }
         }
         else if ( command == 'W' ) // withdraw?
         { int amount = reader.readAmount();
            boolean ok = account.withdraw(amount);
            if ( ok )
            { writer.showTransaction("Terheqja ne $", amount); }
            else { writer.showTransaction("Terheqje ilegale ", amount); }
         }
         else { writer.showTransaction("Komande ilegale " + command); }
